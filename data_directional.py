...
class Dataset:
    ...
    def sample_pair(self, direction: str=None) -> Pair:
        """Sample a minimal pair from the dataset.

        direction: None (random), 'wantto_to_wanna' (types[0]=base, types[1]=src),
                   'wanna_to_wantto' (types[1]=base, types[0]=src)
        """
        # pick types (should differ)
        if direction == 'wantto_to_wanna':
            base_type = self.types[0]
            src_type = self.types[1]
        elif direction == 'wanna_to_wantto':
            base_type = self.types[1]
            src_type = self.types[0]
        else:
    ...
    def sample_batch(
            ...
            direction: Union[str, None]=None) -> Batch:
        """Sample a batch of minimal pairs from the dataset.
        
        direction: None (random + swap, default), 'wantto_to_wanna' (types[0]=base, types[1]=src, no swap),
                   'wanna_to_wantto' (types[1]=base, types[0]=src, no swap)
        """
        pairs = []
        ...
        if model is None:
            while len(pairs) < batch_size // 2:
                pair = self.sample_pair(direction=direction)
                ct = 0
        ...
        # add flipped pairs only if direction is None (default bidirectional)
    ...
    def sample_batches(
            ...
            discard: set[str]=set(), manipulate: Union[str, None]=None,
            direction: Union[str, None]=None) -> list[Batch]:
        """Sample a list of batches of minimal pairs from the dataset."""
        random.seed(seed)
        return [self.sample_batch(tokenizer, batch_size, device, model, discard, manipulate, direction) for _ in range(num_batches)]
