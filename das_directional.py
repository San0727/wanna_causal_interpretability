...
from data_directional import Dataset
...
def experiment(
    ...
    trainset = data_source.sample_batches(tokenizer, batch_size, steps, device, seed=42, manipulate=manipulate, direction=direction)
    print(trainset[0])
    discard = set()
    ...
    evalset = data_source.sample_batches(tokenizer, batch_size, 25, device, seed=eval_seed, discard=discard, manipulate=manipulate, direction=direction)
    ...
    log_file = f"logs/{log_folder}/{NOW}__{short_model_name}__{short_dataset_name}__{manipulate}{direction_str}.json"
